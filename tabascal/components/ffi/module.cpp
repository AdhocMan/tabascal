...
void rfi_transpose_kernel(Tensor1D<const int *> a1, Tensor1D<const int *> a2,
                          Tensor4D<const std::complex<double> *> rfi_amp_fine,
                          Tensor4D<const double *> rfi_phase,
        ...
        for (std::int64_t i_rfi = 0; i_rfi < n_rfi; ++i_rfi) {
          ...
          for (std::int64_t i_f_fine = i_f_fine_begin;
            ...
            for (std::int64_t i_t_fine = i_t_fine_begin;
              ...
              const auto f1 = (val_rfi_vis_grad * std::complex<double>(0, 1) *
                               val_e * val_rfi_amp_1 * std::conj(val_rfi_amp_2))
                                  .real();

              rfi_phase_grad(i_rfi, i_a1, i_f_fine, i_t_fine) += f1;
              rfi_phase_grad(i_rfi, i_a2, i_f_fine, i_t_fine) -= f1;
            }
          }
        }
...
calc_rfi_transpose_cpu_impl(ffi::BufferR1<ffi::S32> a1,
  ...
  rfi_transpose_kernel(a1_tensor, a2_tensor, rfi_amp_fine_tensor,
                       rfi_phase_tensor, rfi_grad_tensor,
